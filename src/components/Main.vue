      ...
      <div>
        ...
        <form @submit.prevent="addTask">
          ...
          <input
            class="input"
            type="text"
            placeholder="for this person..."
            v-model="taskReciever"
            required
          />
          <button
            class="add-button"
        ...
        <ul>
          <li v-for="(task, index) in tasks" v-bind:key="index">
            {{ task.taskName }} <b>for </b> {{ task.taskReciever }}
            <button v-on:click="removeTasks(index)" class="remove">
              Done ✅
            </button>
          </li>
        </ul>
